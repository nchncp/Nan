...
/*$SQLSelectNote = "SELECT A.ANNOTATION_TEXT, A.ANNOTATION_OWNER, DISP.USERNAME, DISP.CHECKED "
	...
	$SQLSelectNote = "SELECT DISTINCT A.ANNOTATION_TEXT, A.ANNOTATION_OWNER, DISP.USERNAME, DISP.CHECKED "
.", DISP.PRESENTER_CHECKED, A.ANNOTATION_UPD_TIMESTAMP, DISP.USER_STATUS, A.SOURCE_IMAGE,R.ROOM_NAME " 

."FROM ANNOTATION_DISP AS DISP, ANNOTATION AS A, ROOM_INFO AS R, FILE_INFO AS F, USER_ACCOUNT AS USR "

."WHERE A.ROOM_NO = '$txtRoomNo' "
."AND A.FILEID = '$hidFileid' "
."AND A.ROOM_NO = R.ROOM_NAME "
."AND DISP.PAGE_NO = A.PAGE_NO "
."AND R.ROOMID = F.ROOMID "
//."AND DISP.OWNER = USR.USERNAME "
//."AND USR.USERID = F.USERID "
."AND DISP.OWNER = A.ANNOTATION_OWNER "
."AND DISP.OWNER = USR.USERNAME "
...
$rsSelectNote = mysqli_query($connect,$SQLSelectNote) or die("ShowPresenterAnnotations.php()-- error:: " . mysqli_error($connect));
...
?>
<input type="hidden" id="hidPresenterPage" name="hidPresenterPage" value="<?= $pagePresenter ?>" />
